...
public class ChunkGetter implements Runnable {
    ...
    private static class ChunkGetterRandom implements IChunkGetterStrategy {
        ...
        public List<ChunkAccess> generateChunks(ServerLevel level) {
            ...
            final int chunkX = level.getRandom().nextInt(2 * maxChunkPos) - maxChunkPos + (int) worldBorder.getCenterX();
            final int chunkZ = level.getRandom().nextInt(2 * maxChunkPos) - maxChunkPos + (int) worldBorder.getCenterZ();
